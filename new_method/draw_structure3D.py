...
参数：
    structures_dir: structures 目录路径（默认为 output/structures）
...
def find_structures_dirs():
    ...
    优先级：
    1. 命令行参数
    2. config/default_config.yaml 中定义的 output.save_dir 下的所有 smiles 目录
    3. ./output/*/structures（默认）
    ...
    if len(sys.argv) > 1:
        ...
        if os.path.exists(arg_path):
            ...
            # 如果是 smiles 目录，查找其下的 structures
            elif os.path.exists(os.path.join(arg_path, 'structures')):
                structures_dirs.append(os.path.join(arg_path, 'structures'))
                print(f"使用命令行参数指定的目录：{arg_path}/structures")
                return structures_dirs
            # 如果是 output 目录，查找所有子目录
            elif os.path.basename(arg_path) == 'output':
                for smiles_dir in os.listdir(arg_path):
                    structures_path = os.path.join(arg_path, smiles_dir, 'structures')
    ...
    if os.path.exists(config_path):
        ...
        # 查找 base_save_dir 下的所有 smiles 目录
        if os.path.exists(base_save_dir):
            for item in os.listdir(base_save_dir):
                smiles_dir = os.path.join(base_save_dir, item)
                structures_dir = os.path.join(smiles_dir, 'structures')
                if os.path.isdir(structures_dir):
                    structures_dirs.append(structures_dir)
    ...
    # 3. 使用默认路径 ./output/*/structures
    output_dir = './output'
    if os.path.exists(output_dir):
        for item in os.listdir(output_dir):
            smiles_dir = os.path.join(output_dir, item)
            structures_dir = os.path.join(smiles_dir, 'structures')
            if os.path.isdir(structures_dir):
                structures_dirs.append(structures_dir)
